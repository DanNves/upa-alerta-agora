    ...
    <main className="min-h-dvh bg-background pb-28">
      ...
      >
        <Link
          to="/"
          aria-label="Voltar"
          className="absolute left-5 top-5 rounded-full bg-white/20 p-2.5 text-white backdrop-blur transition hover:bg-white/30"
        >
        ...
        <button
          ...
          className="absolute right-5 top-5 inline-flex items-center gap-2 rounded-full bg-white/20 px-3.5 py-2.5 text-xs font-semibold text-white backdrop-blur transition hover:bg-white/30"
        >
          <Star className="h-4 w-4 shrink-0" fill={favorito ? "#F59E0B" : "transparent"} stroke={favorito ? "#F59E0B" : "currentColor"} />
        ...
        <div className="absolute inset-x-0 bottom-0 p-6 text-white">
          ...
          <h1 className="mt-1.5 text-2xl font-bold leading-tight tracking-tight">{upa.nome}</h1>
        </div>
      </header>

      {favorito && (
        <div className="mx-auto mt-5 max-w-md px-5">
          <div className="flex items-start gap-2.5 rounded-2xl border border-border bg-muted px-3.5 py-3 text-xs leading-relaxed text-muted-foreground">
            ...
            <span>
              ...
              <strong className="font-semibold text-foreground">baixa</strong> (até 50% da capacidade).
            </span>
          </div>
        </div>
      )}
      ...
      <div className="mx-auto max-w-md space-y-5 px-5 pt-5">
          ...
          <div className="mt-4 grid grid-cols-2 gap-3 text-sm">
            <div className="rounded-2xl bg-muted px-3.5 py-3">
              <div className="flex items-center gap-1.5 text-[11px] font-medium uppercase tracking-wider text-muted-foreground">
                <Clock className="h-3.5 w-3.5 shrink-0" /> Tempo estimado
              </div>
              <div className="mt-1.5 text-lg font-bold tabular-nums text-foreground">
                ~{upa.tempo_estimado} min
              </div>
            </div>
            <div className="rounded-2xl bg-muted px-3.5 py-3">
              <div className="flex items-center gap-1.5 text-[11px] font-medium uppercase tracking-wider text-muted-foreground">
                <MapPin className="h-3.5 w-3.5 shrink-0" /> Distância
              </div>
              <div className="mt-1.5 text-lg font-bold tabular-nums text-foreground">
                {dist.toFixed(1)} km
              </div>
        ...
        {/* Info */}
        <section className="space-y-1 rounded-3xl border border-border bg-card p-5 text-sm shadow-sm">
          ...
          <Row
            ...
            action={
              <button
                ...
                className="inline-flex shrink-0 items-center gap-1.5 rounded-full bg-secondary px-3 py-1.5 text-xs font-semibold text-secondary-foreground transition hover:bg-accent"
              >
                <Copy className="h-3 w-3 shrink-0" /> Copiar
              </button>
            }
          />
          <Row
            ...
            action={
              <a
                ...
                className="inline-flex shrink-0 items-center gap-1.5 rounded-full bg-primary px-3 py-1.5 text-xs font-semibold text-primary-foreground shadow-sm transition hover:opacity-95"
              >
                <Phone className="h-3 w-3 shrink-0" /> Ligar
              </a>
            }
          />
        ...
        <section className="rounded-3xl border border-border bg-card p-5 shadow-sm">
          ...
          <div className="mt-3.5 grid grid-cols-3 gap-2.5">
            {upa.servicos.map((s) => (
              <div
                key={s}
                className="flex flex-col items-center justify-center gap-1.5 rounded-2xl bg-muted px-2 py-3.5 text-center"
              >
                <span className="text-xl leading-none" aria-hidden>{servicoIcon[s] ?? "🏥"}</span>
                <span className="text-[11px] font-semibold leading-tight text-foreground">{s}</span>
              </div>
            ))}
          </div>
        </section>
